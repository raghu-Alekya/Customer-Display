...
// holds value in paise/cents, e.g. 2345
  ...
  Future<void> _calculateBalanceFromPaymentHistory() async {
    try {
      ...
      setState(() {
        ...
        payByCash = payments
            .where((p) => p.paymentMethod.toLowerCase() == TextConstants.cash.toLowerCase())
            .fold(0.0, (sum, p) => sum + p.amount);

        payByCard = payments
            .where((p) => p.paymentMethod.toLowerCase() == TextConstants.card.toLowerCase())
            .fold(0.0, (sum, p) => sum + p.amount);

        payByEbt = payments
            .where((p) => p.paymentMethod.toLowerCase() == TextConstants.ebtText.toLowerCase())
            .fold(0.0, (sum, p) => sum + p.amount);
  ...
  Future<void> _printPaymentHistorySummary() async {
    ...
    print("Active Session: ${_currentPaymentRemainingBalance != null ? 'Yes' : 'No'}");
    if (_currentPaymentRemainingBalance != null) {
      print("Session Balance: \$${_currentPaymentRemainingBalance!.toStringAsFixed(2)}");
    }
    print("=" * 70 + "\n");
  }
...
//  Sync to server
  ...
  void _handlePay() {
    ...
    if (balanceAmount <= 0 &&
        (double.tryParse(amountController.text.replaceAll(TextConstants.currencySymbol, '').trim()) ?? 0) > 0) {

      print("⚠️ DEFENSIVE RESET: balance=0 but amount entered > 0 → forcing reset after possible void");
                                            ...
                                            Expanded(
                                              child: PaymentNumPad(
                                                ...
                                                onDigitPressed: (value) {
                                                  ...
                                                  if (selectedPaymentMethod == TextConstants.ebtText) {
                                                    maxAmount = (min(ebtTotal, effectiveBalance) * 100).toInt();
                                                  } else if (selectedPaymentMethod == TextConstants.card) {
                              ...
                              _buildCouponButton(
                                ...
                                onTap: () {
                                  if (hasEbtItem ||
                                      ...
                                      isPaymentStarted ||
                                      isOrderPending ||
                                      hasOnlyCashbackOrPayoutItems) {   // ✅ BLOCK TAP
                                    return;
                                  }
...
// 🔥 UPDATE DISPLAY
  ...
  Future<void> _handleVoidPayment(BuildContext context, {required bool isPartial}) async {
    // ────────────────────────────────────────────────
    //  0. Early validation
    // ────────────────────────────────────────────────
    ...
    try {
      ...
      print("Void saved in Isar → ID: ${savedVoid.id} | amount: -${voidedAmount.toStringAsFixed(2)}");
  ...
  Future<void> _updatePaymentStatusInHive(int localOrderId, int paymentLocalId, String newStatus) async {
    try {
      final box = StorageProvider.offlineOrders;
      final String hiveKey = localOrderId.toString(); // key is the local order ID
  ...
  // ============================================================
// REPLACE your _showPaymentDialog method with this fixed version
// KEY FIX: Close dialog IMMEDIATELY, then do async work in background
// ============================================================
  ...
  void _showPaymentDialog(
    ...
    void doBackgroundWork() {
      // Run all heavy async work AFTER dialog is already closed
      Future(() async {
        ...
        if (orderId != null && orderId! > 0) {
          ...
          while (retries > 0) {
            ...
            if (pending.isNotEmpty) {
              ...
              print("✅ Background: Marked ${pending.length} payments as completed");
              break;
            }
            retries--;
    ...
    showDialog(
      ...
      builder: (dialogCtx) => PaymentDialog(
        ...
        // ── NO RECEIPT ───────────────────────────────────────
        onNoReceipt: () {
          // ✅ CLOSE DIALOG IMMEDIATELY — no await before this
          Navigator.of(dialogCtx, rootNavigator: false).pop();

          // Do all heavy work in background AFTER dialog closes
          doBackgroundWork();
        ...
        onDone: (selectedOption, {String? email}) {
          ...
          // ── EMAIL: close dialog first, then send email ──────
          if (selectedOption == TextConstants.email &&
              email != null &&
              email.isNotEmpty) {
            ...
            // Do background work and navigate
            doBackgroundWork();
            OrderHelper.isOrderPanelLoaded = false;
            OrderHelper.notifyOrderPanelToRefresh();
            Navigator.pushReplacement(
          ...
          if (selectedOption == TextConstants.print && !Misc.disablePrinter) {
            // Print in background — don't block navigation
            Future(() async {
              await _preparePrintTicket();
              await _printTicket(manual: true);
            });
...
// ============================================================
// ALSO FIX showVoidExitConfirmation → onVoidCancel to close immediately
// ============================================================
