...
class _CompletedOrdersScreenState extends State<CompletedOrdersScreen>
  ...
  static bool? _lastOnlineModeState;
  int get _totalPages {
  ...
  Future<void> _loadCompletedOrders({bool forceRefresh = false}) async {
    ...
    final bool modeChanged = _lastOnlineModeState != null && _lastOnlineModeState != isOnline;
    _lastOnlineModeState = isOnline;
    ...
    if (!isOnline) {
      // ---------------------------------------------------------------
      // OFFLINE MODE: Load directly from SQLite
      // ---------------------------------------------------------------
      if (mounted) {
        setState(() {
          _isLoading = true;
      ...
      try {
        final localOrders = await OfflineHelper.buildOfflineCompletedOrdersFromDb(
          page: 1,
          perPage: 100,
        );
    ...
    if (hasCache && !forceRefresh && cacheFresh) {
      _showOrdersImmediately(_cachedOrders);
      return;
    }

    if (mounted && _cachedOrders.isEmpty) {
      setState(() {
        _isLoading = true;
        _errorMessage = null;
      });
    ...
    try {
      ...
      _cachedOrders = List<CompletedOrder>.from(orders);
      _cachedOrderIds = _cachedOrders.map((o) => o.orderId).toSet();
      _lastFetchTime = DateTime.now();
      ...
      // Fall back to SQLite local orders if online fetch fails
      try {
        final localOrders = await OfflineHelper.buildOfflineCompletedOrdersFromDb(
          page: 1,
          perPage: 100,
      ...
      if (kDebugMode) debugPrint('⚠️ [Refund] Online fetch failed, fallback to local: $e');
    }
  }
