...
class PrinterSettings {
  ...
  BluetoothPrinter? selectedPrinter;
  List<BluetoothPrinter> allPrinters = []; // <<< store all printers
  var printerManager = PrinterManager.instance;
  List<int>? pendingTask;
  final PrinterDBHelper _printerDBHelper = PrinterDBHelper(); ////  get all printers
  ...
  Future<void> loadPrinter() async {
    ...
    if (kDebugMode) {
      print(
          "PrinterSettings - loadPrinter called is Printer loaded: ${selectedPrinter?.deviceName ?? ''}");
    }
  }
  ...
  Future<void> saveSelectedPrinterToDB() async {
    ...
    if (kDebugMode) {
      print(
          ">>>>> PrinterSettings before saveSelectedPrinterToDB: ${selectedPrinter?.deviceName}, ${selectedPrinter?.productId ?? selectedPrinter?.address}, ${selectedPrinter?.vendorId}, ${selectedPrinter?.typePrinter}");
    }
    int printerId = await _printerDBHelper.addPrinterToDB(selectedPrinter!);

    if (kDebugMode) {
      print(
          ">>>>> PrinterSettings after saveSelectedPrinterToDB: at row $printerId, ${selectedPrinter?.deviceName}, ${selectedPrinter?.productId}, ${selectedPrinter?.vendorId}, ${selectedPrinter?.typePrinter}");
    }
  }
  ...
  Future<void> setSelectedPrinterFromDB() async {
    if (kDebugMode) {
      print(
          ">>>>> PrinterSettings before setSelectedPrinterFromDB: ${selectedPrinter?.deviceName}, ${selectedPrinter?.productId}, ${selectedPrinter?.vendorId}, ${selectedPrinter?.typePrinter}");
    }
    var printerDB = await _printerDBHelper.getPrinterFromDB();

    if (printerDB.isEmpty) {
      if (kDebugMode) print(">>>>> PrinterSettings : printerDB is empty");
      return;
    } else if (printerDB.first[AppDBConst.printerDeviceName].toString().isEmpty) {
      return;
    }
    ...
    printer.typePrinter = EnumToString.fromString(
        PrinterType.values, printerDB.first[AppDBConst.printerType]) ??
        PrinterType.usb;
    printer.isBle = false;
    _currentStatus = (printer.typePrinter == PrinterType.bluetooth &&
        printer.address != "")
        ? BTStatus.connected
        : BTStatus.none;
    ...
    if (kDebugMode) {
      ...
      if (printerDB.isNotEmpty) {
        print(
            ">>>>> PrinterSettings Retrieved printerDevice '${printerDB.first[AppDBConst.printerDeviceName]}' from DB");
      }
      print(
          ">>>>> PrinterSettings after setSelectedPrinterFromDB: ${selectedPrinter?.deviceName}, ${selectedPrinter?.productId}, ${selectedPrinter?.vendorId}, ${selectedPrinter?.typePrinter}");
    }
    await Future.delayed(Duration(milliseconds: 2000));
  }
  ...
  Future<void> setSelectedPrinter() async {
    ...
    if (kDebugMode) {
      print(
          ">>>>> PrinterSettings setSelectedPrinter: ${selectedPrinter?.deviceName}, ${selectedPrinter?.productId}, ${selectedPrinter?.vendorId}, ${selectedPrinter?.typePrinter}");
    }
  }
  ...
  Future<Result<BluetoothPrinter>> printTicket(
      List<int> bytes, Generator generator) async {
    if (selectedPrinter == null) return Result.error(Exception(TextConstants.noPrinter));
    var bluetoothPrinter = selectedPrinter!;
    ...
    switch (bluetoothPrinter.typePrinter) {
      ...
      case PrinterType.bluetooth:
        ...
        break;
      case PrinterType.network:
        bool connectedTCP = await printerManager.connect(
            type: bluetoothPrinter.typePrinter,
  ...
  /// ================================
  /// NEW METHOD: LOAD ALL PRINTERS AND STORE
  /// ================================
  Future<void> loadAllPrinters() async {
    ...
    for (var p in printerDB) {
      ...
      printer.typePrinter = EnumToString.fromString(
          PrinterType.values, p[AppDBConst.printerType]) ??
          PrinterType.usb;
      ...
      if (kDebugMode) {
        print(
            "Printer [ID:${printer.id}] Name:${printer.deviceName}, VendorId:${printer.vendorId}, ProductId:${printer.productId}, Type:${printer.typePrinter}");
      }
    }
  }
  ...
  /// ================================
  Future<void> printToAllPrinters(List<int> bytes, Generator generator) async {
    if (allPrinters.isEmpty) await loadAllPrinters();

    if (allPrinters.isEmpty) {
      if (kDebugMode) print("Printer list is empty. Cannot print.");
      return;
    }

    for (var printer in allPrinters) {
      selectedPrinter = printer;
      await printTicket(bytes, generator);
      if (kDebugMode) print("Printed to ${printer.deviceName}");
    }
  }
}

/// ======================================
/// BluetoothPrinter Class (unchanged)
/// ======================================
class BluetoothPrinter {
  int? id;
  String? deviceName;
