...
class StoreMessagingService {
  ...
  // MQTT PAYLOAD LIMITS
  // ---------------------------------------------------------------------------
  static const int _maxSinglePublishBytes = 4096;
  static const int _maxChunkPublishBytes = 4096;
  static const int _initialChunkDataSize = 3800;
  static const Duration _publishDelay = Duration(milliseconds: 20);

  final Map<String, int> _lastChunkCountByTopic = {};
  final Map<String, int> _lastSequenceByTopic = {};

  final Map<String, String> _lastPublishedFingerprint = {};
  final Map<String, Map<String, dynamic>> _pendingLatestDataByTopic = {};
  ...
  dynamic _canonicalize(dynamic val) {
    if (val is Map) {
      final sortedKeys = val.keys.map((e) => e.toString()).toList()..sort();
      return {for (final k in sortedKeys) k: _canonicalize(val[k])};
    } else if (val is List) {
  ...
  String _fingerprint(Map<String, dynamic> data) {
    final copy = Map<String, dynamic>.from(data)..remove('sequence');
    return jsonEncode(_canonicalize(copy));
  }
  ...
  Future<void> _publishJsonInternal(
    ...
    // SMALL PAYLOAD → direct publish with pre-filled chunk 0
    if (payloadBytes.length <= _maxSinglePublishBytes) {
      try {
        final metaPayload = jsonEncode({
          'sequence': sequence,
        ...
        // 1. Publish chunk 0 FIRST so receiver buffer has data before meta arrives
        await _publishRaw('$baseTopic/chunk/$sequence/0', payload, retain: false);
        // 2. Publish meta SECOND to trigger instant assembly without waiting/flickering
        await _publishRaw('$baseTopic/meta', metaPayload, retain: false);
        // 3. Publish base topic for direct listeners
        await _publishRaw(baseTopic, payload, retain: false);

        print('✅ [MQTT] Direct & single-chunk publish complete (seq=$sequence)');
      } catch (e, st) {
        print('❌ [MQTT] Direct publish failed: $e');
        print(st);
      }
  ...
  // ---------------------------------------------------------------------------
  // SERIALIZED PUBLISH QUEUE (with coalescing)
  // ---------------------------------------------------------------------------
  ...
  Future<void> _publishJson(
    ...
    _pendingLatestDataByTopic[baseTopic] = data;

    final next = _publishQueue.then((_) async {
      final latest = _pendingLatestDataByTopic.remove(baseTopic);
      if (latest != null) {
        await _publishJsonInternal(baseTopic, latest);
      }
    });
  ...
  Future<void> publishState(CartState state) async {
    final currentSession = '${state.orderId}-${state.screen}';
    if (_lastSessionId != currentSession) {
    ...
    client.updates?.listen(
        ...
        for (final event in events) {
          try {
            ...
            // Force clear fingerprint cache so instant auto-publish is never swallowed
            _lastPublishedFingerprint.clear();
