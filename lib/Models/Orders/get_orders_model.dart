...
//     required this.isRefundItem, required double unitPrice,
//   });
//
...
class OrderModel {
  ...
  factory OrderModel.fromJson(Map<String, dynamic> json) {
    ...
    final orderDiscountStr = getOrderMetaValue('_discount_amount');
    final orderLevelAutoDiscountAmount = double.tryParse(orderDiscountStr ?? '0') ?? 0.0;
    final refundTotal = double.tryParse(json['refund_total']?.toString() ?? '0') ?? 0.0;
    final refundOrderTotal = double.tryParse(json['refund_order_total']?.toString() ?? '0') ?? 0.0;
    final netPayment = double.tryParse(json['net_payment']?.toString() ?? '0') ?? 0.0;
  ...
  double get totalMultipackDiscount => lineItems.fold(0.0, (sum, item) => sum + item.multipackDiscountAmount);
  double get totalAutoDiscount => lineItems.fold(0.0, (sum, item) => sum + item.autoDiscountAmount);
  double get totalComboDiscount => lineItems.fold(0.0, (sum, item) => sum + item.comboDiscountAmount);
  double get totalDisplayAutoDiscount => lineItems.fold(0.0, (sum, item) => sum + item.displayAutoDiscountAmount);
  double get totalCombinedAutoDiscount => orderLevelAutoDiscountAmount + totalAutoDiscount;
  double get totalAllDiscounts {
    final discount = double.tryParse(discountTotal) ?? 0.0;
    return discount + totalMultipackDiscount + totalAutoDiscount + orderLevelAutoDiscountAmount + totalComboDiscount + totalDisplayAutoDiscount;
  }
...
class LineItem {
  ...
  factory LineItem.fromJson(Map<String, dynamic> json) {
    ...
    // Weighted Item Parsing
    final double? weightQty = double.tryParse(getMetaValue('_weight_qty') ?? '');
    final String? weightUnit = getMetaValue('_weight_unit');
    final double? unitPrice = double.tryParse(getMetaValue('_unit_price') ?? '');
    ...
    if (autoDiscountApplied && autoDiscountAmount > 0) {
      ...
      final String? displayAutoStr = getMetaValue('auto_discount_amount') ??
          _extractDisplayMetaValue(metaList, 'auto_discount_amount');
      displayAutoDiscountAmount = double.tryParse(displayAutoStr ?? '0') ?? 0.0;
    }
  ...
  // Weighted Item Helpers
  bool get isWeighted => weightQty != null && weightQty! > 0;

  String get displayQuantity => isWeighted && weightQty != null
      ? weightQty!.toStringAsFixed(3)
      : quantity.toString();

  String get displayUnit => isWeighted && weightUnit != null ? weightUnit! : '';

  // Discount Helpers (unchanged)
  String? get primaryDiscountType {
    if (autoDiscountApplied && autoDiscountAmount > 0) return 'auto';
    if (comboDiscountApplied && comboDiscountAmount > 0) return 'mixmatch';
    if (multipackApplied && multipackDiscountAmount > 0) return 'multipack';
  ...
  double get totalDiscountAmount =>
      multipackDiscountAmount + autoDiscountAmount + comboDiscountAmount + displayAutoDiscountAmount;
}
