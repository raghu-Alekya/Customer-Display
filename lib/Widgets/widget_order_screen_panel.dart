...
class _OrderScreenPanelState extends State<OrderScreenPanel>
  ...
  void initState() {
    ...
    OrderHelper.orderPanelRefreshNotifier.addListener(
      _orderPanelRefreshListener!,
    );
  ...
  Future<void> _fetchWooOrder(int serverId) async {
    try {
      final woo = await _orderRepository.getOrderdata(
        orderId: serverId.toString(),
      );
  ...
  void _processPaymentList(List<PaymentListModel> payments) {
    ...
    setState(() {
      ...
      tenderAmount = (payByCash + payByOther + ebtAmount).clamp(
        0.0,
        double.infinity,
      );
      ...
      balanceAmount = (effectiveTotal - tenderAmount).clamp(
        0.0,
        double.infinity,
      );
  ...
  Future<void> _loadBalanceFromLocalPayment() async {
    ...
    try {
      ...
      if (paymentCount > 0) {
        ...
        double computedChange = (totalPaid - netPay).clamp(
          0.0,
          double.infinity,
        );
        ...
        try {
          ...
          if (paymentCount > 0) {
            ...
            if (remaining != null) {
              ...
              if (netPay != null) {
                balanceAmount = (netPay.toDouble() - tenderAmount).clamp(
                  0.0,
                  double.infinity,
                );
              }
  ...
  Future<void> fetchOrder() async {
    ...
    List<Map<String, dynamic>> ordersData = await orderHelper.getOrderById(
    ...
    if (ordersData.isNotEmpty) {
      ...
      final offlineRaw = await StorageProvider.offlineOrders.get(
        widget.activeOrderId.toString(),
      );
  ...
  void dispose() {
    if (_orderPanelRefreshListener != null) {
      OrderHelper.orderPanelRefreshNotifier.removeListener(
        _orderPanelRefreshListener!,
      );
    }
  ...
  Widget buildCurrentOrder() {
    ...
    if (!isGeneratedOnly) {
      metaCouponVal = double.tryParse(
            order['couponValue']?.toString() ??
                ...
                order['discount']
                    ?.toString() ?? // From standard WC/POS API field
    ...
    final double remainingAmount = (netTotal.toDouble() + orderTax).clamp(
      0.0,
      double.infinity,
    );
    ...
    */
    ...
    bytes += ticket.row([
      ...
      PosColumn(
        text: formatCurrency(
          0.0,
        ), // Need to map service charges properly (future build)
        width: 4,
        styles: PosStyles(align: PosAlign.right),
      ),
    ]);
    ...
    double printChange = (effectiveTender - printNetPayable).clamp(
      0.0,
      double.infinity,
    );
