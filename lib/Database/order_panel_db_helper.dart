...
class OrderHelper {
  ...
  Future<void> loadData() async {
    ...
    if (kDebugMode) {
      print("🔍 loadData: Hive box size: ${allOfflineOrders.length}");
      print("🔍 loadData: Hive keys: ${allOfflineOrders.keys.toList()}");
    }
    ...
    if (validEntries.isNotEmpty) {
      ...
      localOrderIds = localOrders
          .map((map) {
        final rawId = map['order_id'] ?? map['id'] ?? map[AppDBConst.orderServerId];
      ...
      if (kDebugMode) {
        print("🔍 loadData: initial currentActiveOrderId: $currentActiveOrderId");
        print("🔍 loadData: localOrderIds: $localOrderIds");
        if (localOrders.isNotEmpty) {
          print("🔍 loadData: first order_id type: ${localOrders.first['order_id'].runtimeType}");
          print("🔍 loadData: first id type: ${localOrders.first['id'].runtimeType}");
        }
      }
      ...
      if (currentActiveOrderId == null ||
        ...
        if (kDebugMode) print("🔄 loadData: currentActiveOrderId ($currentActiveOrderId) invalid or not in $localOrderIds - attempting restore");
        ...
        if (checkpointId != null &&
          ...
          if (kDebugMode) print("🔄 loadData: Final Fallback to newest order: $currentActiveOrderId (checkpoint was: $checkpointId)");
        }
...
// 🔥 SYNC CUSTOMER DISPLAY AFTER DELETE
  ...
  Future<void> setActiveOrder(int? orderId) async {
    if (kDebugMode) print("🎯 setActiveOrder: $activeOrderId -> $orderId");
    activeOrderId = orderId;
  ...
  Future<bool> localOrderExistsForActiveUser(int orderId) async {
    ...
    if (kDebugMode) {
      print("🔍 localOrderExistsForActiveUser: orderId=$orderId, rawFound=${raw != null}");
    }
    if (raw != null && raw is Map) {
